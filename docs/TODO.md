...
- [x] Re-export `Component`, `Theme`, `KeyEvent`, `KeyHandler`, `onKey`, `terminalHandle` from `packages/tui/src/index.ts` so cog can import them. (Chunk 5)
- [x] **M3.8 — cog wired to TUI.** Replaced the M2.5 stdout dumper with `new TUI(); tui.start(); for await { handleEvent }; tui.stop()`. Added a **hold-until-exit** after the for-await ends — the TUI stays open after the mock's `stop` event fires until the user presses Esc or Ctrl+C, so visual verification is possible.
- [x] **Lifecycle fixes from M3.8 verification:** added `Renderer.unmount()` (cancels pending redraw timer); `TUI.stop()` now calls `process.stdin.pause()` (releases stdin so Node can exit), emits `\x1b[0m\r\n` after alt-screen exit (resets styling + cursor to clean row), and unmounts the renderer first. Without these, the process hung after the scenario completed.
- [x] **handleEvent bug fixes:** moved `scheduleRedraw()` to the top of `handleEvent` (was at the bottom — never reached during the `permission_ask` `await`, so the prompt UI didn't appear until the user pressed a key). Added `setLabel(null)` to the `stop` case so the activity line auto-clears at end of stream (otherwise the error scenario kept "thinking" visible after the error).
