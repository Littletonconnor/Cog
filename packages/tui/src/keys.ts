 ...
 * A single buffer may contain multiple keypresses (e.g. pasted text),
 ...
 *   0x1b 0x5b 0x5a       → tab (back)  — Shift+Tab (CSI Z)
 *   0x1b (alone)         → esc
    ...
    if (byte === 0x03) {
      ...
      if (chunk[i + 1] === 0x5b) {
        ...
        if (character === 0x41) {
          ...
          i += 3;
        } else if (character === 0x43) {
          events.push({ type: 'arrow', dir: 'right' });
          i += 3;
        } else if (character === 0x44) {
          events.push({ type: 'arrow', dir: 'left' });
          i += 3;
        } else if (character === 0x5a) {
          events.push({ type: 'tab', dir: 'back' });
          i += 3;
        } else {
      ...
      events.push({ type: 'tab', dir: 'forward' });
      i++;
    } else {
      i++;
    }
