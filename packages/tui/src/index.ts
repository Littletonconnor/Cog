...
class TUI implements Component {
  ...
  render(width: number, theme: Theme): string[] {
    ...
    const promptLines = isPromptActive ? this.permissionPrompt.render(width, theme) : [];
  ...
  start() {
    ...
    this.unsubscribeKey = onKey((event) => {
      const target = this.permissionPrompt.isActive() ? this.permissionPrompt : this.inputBox;
