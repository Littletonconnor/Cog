...
export class MockProvider implements Provider {
  ...
  async *stream(input: ProviderInput): AsyncIterable<StreamEvent> {
    const scenario = await MockProvider.loadScenarioFile(this.filePath);
    MockProvider.validateScenarioFile(scenario, this.filePath);
  ...
  private static async loadScenarioFile(filePath: string) {
  ...
  private static validateScenarioFile(
    parsed: unknown,
    filePath: string,
